...
until it lands. That is deliberate: the firehose never blocks on DNS. The script
...
only if it has requested crawl there, so it is worth testing before demoing, because a
