...
export async function createDhtNode(opts: {
  ...
  // Default 127.0.0.1: on some macOS / VPN setups, 0.0.0.0 triggers ERR_NO_VALID_ADDRESSES.
...
export async function advertiseDescriptor(node: DhtNode, descriptorKey: string): Promise<void> {
  try {
    const cid = descriptorToCid(descriptorKey)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const dht = (node.services as any).dht
    if (dht?.provide) await dht.provide(cid)
  } catch (err) {
    console.warn(`DHT provide failed for ${descriptorKey}:`, (err as Error).message)
  }
}
